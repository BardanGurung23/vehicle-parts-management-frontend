...
export function InvoiceDocument({
  ...
  return (
    <div
      className={`overflow-hidden rounded-[28px] bg-black text-white border border-[#323332] shadow-level2${className ? ` ${className}` : ""}`}
    >
        ...
        <div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-500">
              Parts and service sale summary
            </p>
          </div>
          ...
          <div className="space-y-2 text-sm text-white">
            ...
            <div className="flex justify-between gap-6">
              ...
              <span
                className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${getInvoiceStatusClasses(sale.paymentStatus)}`}
              >
                {sale.paymentStatus}
              </span>
            </div>
              ...
              {sale.items.map((item) => {
                ...
                return (
                  <tr key={`${sale.saleId}-${item.partId}`}>
                    <td className="border-b border-slate-100 px-4 py-4 font-medium text-white">
                      {item.partName}
                    </td>
                    <td className="border-b border-slate-100 px-4 py-4 text-right text-white">
                      {item.quantity}
                    </td>
                    <td className="border-b border-slate-100 px-4 py-4 text-right text-white">
                      {formatInvoiceCurrency(item.unitPrice)}
                    </td>
                    <td className="border-b border-slate-100 px-4 py-4 text-right font-medium text-white">
        ...
        <div className="mt-8 flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
          <div className="max-w-xl space-y-2">
            ...
            <p className="text-sm leading-6 text-white">
              {sale.notes ?? "No additional notes were added to this invoice."}
            </p>
          </div>

          <div className="w-full max-w-sm rounded-2xl bg-[#121313] p-5">
            <div className="space-y-3 text-sm">
              <div className="flex items-center justify-between gap-4">
                <span className="text-slate-500">Subtotal</span>
                <span className="font-medium text-white">
                  {formatInvoiceCurrency(sale.subtotal)}
                </span>
              </div>
              <div className="flex items-center justify-between gap-4">
                <span className="text-slate-500">Discount</span>
                <span
                  className={`font-medium ${sale.discountAmount > 0 ? "text-emerald-700" : "text-white"}`}
                >
