...
const routeMeta: Record<string, { title: string; description: string }> = {
  "/app": { title: "Dashboard", description: "Monitor operations at a glance." },
  "/app/staff": { title: "Staff Management", description: "Create and manage staff accounts." },
  "/app/parts": { title: "Parts Management", description: "Manage inventory, pricing and stock levels." },
  "/app/vendors": { title: "Vendors", description: "Manage supplier records and vendor contact details." },
  "/app/purchase-invoices": { title: "Purchase Invoices", description: "Record vendor stock receipts and increase inventory." },
  "/app/reports/financial": { title: "Financial Reports", description: "Review revenue, discounts, purchase spend, and gross profit." },
  "/app/reports/customers": { title: "Customer Reports", description: "Review regular customers, high spenders, and pending credit accounts." },
  "/app/appointments": { title: "Appointments", description: "Review scheduled appointments and update service status." },
  "/app/book-appointment": { title: "Book Appointment", description: "Schedule a service for your vehicle." },
  "/app/my-appointments": { title: "My Appointments", description: "View your booked service appointments and their status." },
  "/app/shop": { title: "Shop Parts", description: "Browse and purchase vehicle parts." },
  "/app/my-sales": { title: "My Purchases", description: "View your past purchases and service history." },
  "/app/request-part": { title: "Request Part", description: "Request parts not available in our inventory." },
  "/app/my-part-requests": { title: "My Part Requests", description: "Track the status of your part requests." },
  "/app/part-requests": { title: "Part Requests", description: "Manage customer part requests and update their status." },
  "/app/my-reviews": { title: "My Reviews", description: "View and manage your reviews." },
  "/app/customers/register": { title: "Customer Registration", description: "Create staff-managed customer records with an initial vehicle." },
  "/app/register-customer": { title: "Customer Registration", description: "Create staff-managed customer records with an initial vehicle." },
  "/app/customers/search": { title: "Customer Search", description: "Search customer records by ID, phone, vehicle number, or name." },
  "/app/profile": { title: "My Profile", description: "Review your customer profile and linked vehicles." },
  "/app/profile/vehicles": { title: "My Vehicles", description: "Add and remove vehicles linked to your account." },
};

function getRouteMeta(pathname: string): { title: string; description: string } {
  const exact = routeMeta[pathname];
  if (exact) return exact;
  if (pathname.startsWith("/app/customers/") && !pathname.includes("/register") && !pathname.includes("/search")) {
    return { title: "Customer Details", description: "Review customer profiles, vehicles, and service history." };
  }
  return { title: "Dashboard", description: "Monitor operations at a glance." };
}

type NavItem = { label: string; to: string; icon: React.ElementType; show: boolean };
...
export function AppLayout() {
  ...
  const navGroups: NavGroup[] = [
    { label: "Main", items: [{ label: "Dashboard", to: "/app", icon: LayoutDashboard, show: true }] },
    {
      label: "Appointments",
      items: [
        { label: "Book Appointment", to: "/app/book-appointment", icon: CalendarCheck, show: isCustomer },
    ...
    },
    {
      label: "Parts",
      items: [
        { label: "Shop Parts", to: "/app/shop", icon: ShoppingCart, show: true },
        { label: "Request Part", to: "/app/request-part", icon: ClipboardList, show: isCustomer },
    ...
    },
    {
      label: "Profile",
      items: [
        { label: "My Profile", to: "/app/profile", icon: UserCircle, show: isCustomer },
        { label: "Manage Vehicles", to: "/app/profile/vehicles", icon: Truck, show: isCustomer },
      ],
    },
    {
      label: "Customers",
    ...
    },
    {
      ...
      items: [
        ...
        { label: "Appointments", to: "/app/appointments", icon: CalendarCheck, show: isAdmin },
  ...
  const SidebarNav = ({ collapsed }: { collapsed: boolean }) => (
    <nav className="flex-1 overflow-y-auto px-2 py-4 space-y-5" aria-label="Main navigation">
      {navGroups
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full" ref={drawerRef} tabIndex={-1}>
      <div className={`flex items-center shrink-0 glass ${isSidebarCollapsed ? "justify-center h-16" : "gap-3 px-4 h-16"}`}>
        ...
        {!isSidebarCollapsed && (
          <div className="min-w-0">
            <h1 className="text-sm font-bold text-on-surface tracking-tight">Autonix</h1>
            <p className="text-[10px] font-medium text-on-surface-variant uppercase tracking-wider">Access Console</p>
          </div>
        )}
      </div>
      ...
      <div className={`shrink-0 ${isSidebarCollapsed ? "p-2 flex flex-col items-center gap-2" : "p-3"}`}>
        <div className={`flex items-center ${isSidebarCollapsed ? "flex-col gap-2" : "gap-3"}`}>
          ...
          <button
            type="button"
            onClick={logout}
            className="p-2 rounded-full text-on-surface-variant hover:bg-error-container hover:text-error transition-colors shrink-0"
            aria-label="Sign out"
          >
  ...
  return (
    <div className="min-h-screen bg-surface-dim md:flex">
      ...
      <div className="md:flex-1 md:flex md:flex-col md:min-w-0">
        <header className="sticky top-0 z-30 glass shadow-level2" role="banner">
          <div className="flex items-center justify-between gap-4 px-4 h-16">
            <div className="flex items-center gap-3 min-w-0">
              ...
              >
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" className={`transition-transform duration-200 ${isSidebarCollapsed ? "rotate-180" : ""}`}>
              ...
              <div className="min-w-0">
                <h2 className="text-title-large text-on-surface truncate">{title}</h2>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 shrink-0">
              ...
              <div className="flex items-center gap-3 pl-3">
                ...
                <div className="hidden sm:block text-sm">
                  <p className="font-medium text-on-surface">{user?.fullName}</p>
        ...
        <main id="main-content" role="main" className="flex-1 p-4 md:p-6 lg:p-8 page-enter bg-[radial-gradient(ellipse_at_top_right,_rgba(208,188,255,0.03),transparent_50%)]">
          <Outlet />
        </main>
      </div>
    </div>
